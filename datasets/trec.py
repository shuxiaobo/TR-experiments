...
from datasets.binary import BinaryClassifierEval
...
class TREC(object):
    def __init__(self, args, is_train = True, seed=1111):
        ...
        self.seed = seed
        # super(TREC, self).__init__(args = args, is_train = is_train)
        self.train = self.loadFile(os.path.join(args.tmp_dir, self.__class__.__name__, args.train_file))
        self.test = self.loadFile(os.path.join(args.tmp_dir, self.__class__.__name__, args.test_file))
