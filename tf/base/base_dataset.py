...
class BaseDataset(object, metaclass = abc.ABCMeta):
    ...
    def words_to_char_ids(self, sentence, char_dict, tokenizer = default_tokenizer):
        ...
        Turn the sentence to char ids
            ...
            return:         [[1], [2, 4, 3, 5],]
        """
        return [[char_dict.get(ch, self.CHAR_UNK_ID) for ch in word[:self.args.max_char_len] + self._CHAR_PAD * (self.args.max_char_len - len(word))] for word
                in
        ...
        with gfile.FastGFile(data_file) as f:
            for line in f:
                ...
                if max_count and counter > max_count:
                    break
                tokens = [t for t in process_tokens(tokenizer(line.rstrip('\n')))]
    ...
    def prepare_dict(self, file_name):
        ...
        word2id = self.prepare_dictionary(data = self.data_x, dict_path = file_name, exclude_n = self.args.skip_top, max_size = self.args.num_words)
