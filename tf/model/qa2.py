    ...
    def create_model(self):
        ...
        with tf.variable_scope('doc_encoder') as scp:
            ...
            for i in range(self.args.num_layers):
                ...
                # AOA atted
                att = tf.nn.softmax(tf.squeeze(special_math_ops.einsum('bij,bjk->bik', tf.concat(doc_outputs, -1), tf.expand_dims(
                    tf.concat([query_last_states_concat[2 * i][0], query_last_states_concat[2 * i + 1][0]], -1), -1)), -1), -1)
                doc_output_con = tf.concat(doc_outputs, -1) * tf.expand_dims(att, -1)
