        ...
        with tf.variable_scope('encoder') as s:
            ...
            if self.args.bidirectional:
                cell_fw = MultiRNNCell([
                    ModifiedRNNCell(num_units = self.args.hidden_size, activation = nn_ops.relu) for _ in range(self.args.num_layers)])
                ...
                # cell_bw = [
                #     IndRNNCell(num_units = self.args.embedding_dim, recurrent_max_abs = RECURRENT_MAX_ABS, recurrent_kernel_initializer = bw_initializer)
                #     for _ in range(self.args.num_layers)]
