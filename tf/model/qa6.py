...
class QA6(ModelBase):
    ...
    def create_model(self):
        ...
        with tf.variable_scope("query_encoder") as scp:
            query_embed = ContextEmbedding(method = 'concat')(x = query, embedding_matrix = embedding_matrix)
            if self.args.use_char_embedding:
        ...
        with tf.variable_scope('doc_encoder', reuse = tf.AUTO_REUSE) as scp:
            doc_embed = ContextEmbedding(method = 'concat')(x = document, embedding_matrix = embedding_matrix)
            ...
            doc_inputs = doc_embed  # tf.nn.dropout(tf.concat([doc_embed, qry_encoded_dupli], -1), keep_prob = keep_prob)
            ...
            for i in range(self.args.num_layers):
                ...
                doc_outputs, doc_last_states = tf.nn.bidirectional_dynamic_rnn(cell_fw = cell_fw, cell_bw = cell_bw, inputs = doc_inputs,
                                                                               ...
                                                                               initial_state_fw = query_last_states_concat[2 * i],
        ...
        with tf.variable_scope("alt_encoder", reuse = tf.AUTO_REUSE) as scp:
            ...
            context_embed_fun = ContextEmbedding(method = 'concat')
            for j in range(3):
                alt_last_states_concat_tmp = list()
                alt_outputs_concat_tmp = list()
                alter_embed = context_embed_fun(embedding_matrix = embedding_matrix, x = alterative[:, j])
                alter_input = alter_embed
                for i in range(num_layers):
                    ...
                    alter_input = tf.concat([alter_embed, tf.concat(alter_outputs, -1)], -1)
        ...
        with tf.variable_scope('infer_classify') as scp:
            alter_encoded = tf.transpose(tf.concat(alt_last_states_concat, 0), perm = [1, 0, 2])
            r1 = tf.einsum('bij,bjk->bik', alter_encoded, tf.expand_dims(doc_last_states_dropped, -1))
            r2 = tf.einsum('bij,bjk->bik', alter_encoded, tf.expand_dims(query_outputs_max, -1))
            r3 = tf.einsum('bij,bjk->bik', alter_encoded, tf.expand_dims(doc_last_states_dropped - query_outputs_max, -1))
            r4 = tf.einsum('bij,bjk->bik', alter_encoded, tf.expand_dims(doc_last_states_dropped * query_outputs_max, -1))
            result = tf.squeeze(r1 + r2 + r3 + r4)
            # feature = tf.concat(
            #     [doc_last_states_dropped, query_outputs_max, doc_last_states_dropped - query_outputs_max, doc_last_states_dropped * query_outputs_max], -1)
            # embed_w = tf.get_variable('embed_w', shape = [feature.get_shape()[-1], alter_encoded.get_shape()[-2]])
            # result = tf.reduce_sum(tf.matmul(tf.expand_dims(math_ops.matmul(feature, embed_w), 1), alter_encoded), 1)
