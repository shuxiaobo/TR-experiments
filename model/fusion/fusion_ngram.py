...
class FusionNGramRNN(BaseModel):
    ...
    def __init__(self, args, hidden_size, embedding_size, vocabulary_size, rnn_layers = 1, bidirection = False, kernel_size = 3, stride = 1, num_class = 2):
        super(FusionNGramRNN, self).__init__(args = args)
        self.args = args
        ...
        self.linear = nn.Linear(hidden_size * 2 * 3 if bidirection else hidden_size, num_class, bias = False)
    ...
    def forward(self, x, y):
        ...
        for i in range(3):
            ...
            rnn_inputs = torch.cat([x_embed, expanded_out, rnn_out], -1)
